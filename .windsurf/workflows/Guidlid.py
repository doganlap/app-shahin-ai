...
stage_inputs, stage_outputs, hard_dependencies, soft_dependencies, and recompute_triggers (e.g., “if scope changes → regenerate control mapping and assessment templates”)._
...
public class OnboardingRedirectMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, ICurrentUserService currentUserService, ITenantContextService tenantContextService)
    {
        if (context.User.Identity?.IsAuthenticated == true)
        {
            ...
            if (tenant != null &&
                ...
                context.Request.Path != "/onboarding/wizard/fast-start")
            {
...
// 2. OnboardingWizardProgressService.cs
public interface IOnboardingWizardProgressService
{
    Task<int> GetProgressPercentAsync(Guid tenantId);
    Task<bool> IsMissionCompleteAsync(Guid tenantId, string missionKey);
