    ...
    public object? Resolve(object obj, string path)
    {
        ...
        try
        {
            ...
            foreach (var part in parts)
            {
                ...
                // Handle array index notation: Items[0] or Items[0].Property
                ...
                if (part.Contains('[') && part.EndsWith("]"))
                {
                    ...
                    propertyName = part.Substring(0, bracketIndex);
                ...
                {
                    current = dict.Contains(propertyName) ? dict[propertyName] : null;
                    if (current != null && arrayIndex.HasValue)
                    {
                        // Handle dictionary value as array
                        if (current is System.Collections.IList list && arrayIndex.Value >= 0 && arrayIndex.Value < list.Count)
                        {
                ...
                if (arrayIndex.HasValue && current != null)
                {
                    ...
                    {
                        if (arrayIndex.Value >= 0 && arrayIndex.Value < array.Length)
                        {
                            current = array.GetValue(arrayIndex.Value);
                        }
                        else
                        {
                            return null;
                    ...
                    {
                        // Not an array/collection, but index was specified
                        return null;
                    }
                }
            }
