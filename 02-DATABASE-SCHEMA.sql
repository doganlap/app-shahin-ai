...
CREATE INDEX idx_products_code ON grc.products(code);
CREATE INDEX idx_products_category ON grc.products(category);
CREATE INDEX idx_products_active ON grc.products(is_active) WHERE is_active = TRUE;

-- Product Features Table
CREATE TABLE grc.product_features (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    product_id UUID NOT NULL REFERENCES grc.products(id) ON DELETE CASCADE,
...
CREATE INDEX idx_product_features_product ON grc.product_features(product_id);
CREATE INDEX idx_product_features_code ON grc.product_features(feature_code);

-- Product Quotas Table
CREATE TABLE grc.product_quotas (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    product_id UUID NOT NULL REFERENCES grc.products(id) ON DELETE CASCADE,
    ...
    is_deleted BOOLEAN NOT NULL DEFAULT FALSE,
    extra_properties JSONB,
    UNIQUE(product_id, quota_type)
);

CREATE INDEX idx_product_quotas_product ON grc.product_quotas(product_id);
CREATE INDEX idx_product_quotas_type ON grc.product_quotas(quota_type);

-- Pricing Plans Table
CREATE TABLE grc.pricing_plans (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    product_id UUID NOT NULL REFERENCES grc.products(id) ON DELETE CASCADE,
...
CREATE INDEX idx_pricing_plans_product ON grc.pricing_plans(product_id);
CREATE INDEX idx_pricing_plans_active ON grc.pricing_plans(is_active) WHERE is_active = TRUE;

-- Tenant Subscriptions Table
CREATE TABLE grc.tenant_subscriptions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL,
    product_id UUID NOT NULL REFERENCES grc.products(id),
    pricing_plan_id UUID REFERENCES grc.pricing_plans(id),
...
CREATE INDEX idx_tenant_subscriptions_tenant ON grc.tenant_subscriptions(tenant_id);
CREATE INDEX idx_tenant_subscriptions_product ON grc.tenant_subscriptions(product_id);
CREATE INDEX idx_tenant_subscriptions_status ON grc.tenant_subscriptions(status);
CREATE INDEX idx_tenant_subscriptions_active ON grc.tenant_subscriptions(tenant_id, status) WHERE status = 'Active';

ALTER TABLE grc.tenant_subscriptions ENABLE ROW LEVEL SECURITY;
CREATE POLICY tenant_isolation_tenant_subscriptions ON grc.tenant_subscriptions
    USING (tenant_id = current_setting('app.tenant_id', true)::UUID);

-- Quota Usages Table
CREATE TABLE grc.quota_usages (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL,
    quota_type VARCHAR(50) NOT NULL,
    current_usage DECIMAL(18,2) NOT NULL DEFAULT 0,
    reset_date DATE,
...
CREATE INDEX idx_quota_usages_tenant ON grc.quota_usages(tenant_id);
CREATE INDEX idx_quota_usages_type ON grc.quota_usages(quota_type);
CREATE INDEX idx_quota_usages_reset ON grc.quota_usages(reset_date) WHERE reset_date IS NOT NULL;

ALTER TABLE grc.quota_usages ENABLE ROW LEVEL SECURITY;
CREATE POLICY tenant_isolation_quota_usages ON grc.quota_usages
    USING (tenant_id = current_setting('app.tenant_id', true)::UUID);
