...
{
    private readonly IRepository<RiskEntity, Guid> _riskRepository;
    private readonly IRepository<RiskTreatment, Guid> _treatmentRepository;

    public RiskAppService(
        IRepository<RiskEntity, Guid> riskRepository,
    ...
    public async Task<RiskDto> GetAsync(Guid id)
    {
        ...
        var dto = ObjectMapper.Map<RiskEntity, RiskDto>(risk);
    ...
    public async Task<List<RiskDto>> GetListAsync(GetRiskListInput input)
    {
        ...
        return ObjectMapper.Map<List<RiskEntity>, List<RiskDto>>(risks);
    }
}
