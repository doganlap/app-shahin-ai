...
public class AssessmentAppService : ApplicationService, IAssessmentAppService
{
    ...
    public async Task<AssessmentDto> CreateAsync(CreateAssessmentInput input)
    {
        ...
        // Optional quota check - only if quota service is available and tenant has subscription
        if (tenantId.HasValue && _quotaEnforcementService != null)
        {
            try
            {
            ...
            {
                // Quota check failed, proceed anyway (quota service may not be configured)
            }
        }
        ...
        if (tenantId.HasValue && _quotaEnforcementService != null)
        {
            ...
            {
                // Quota reservation failed, ignore
            }
        }
